...
def schema_check(app_configs, **kwargs):
    ...
    try:
        ...
        errors.append(
            Error(f'Schema generation threw exception "{exc}"', id='drf_spectacular_extended.E001')
        )
    if GENERATOR_STATS:
        for w in GENERATOR_STATS._warn_cache:
            errors.append(Warning(w, id='drf_spectacular_extended.W001'))
        for e in GENERATOR_STATS._error_cache:
            errors.append(Warning(e, id='drf_spectacular_extended.W002'))
    return errors
